...
def build_year_config():
    ...
    try:
        ...
        astro_tides = [float(x) for x in os.environ.get('ASTRONOMICAL_HIGH_TIDES').split(",")]
        if len(valid_years) < 2 or len(valid_years) != len(astro_tides):
            raise RuntimeError("VALID_YEARS and ASTRONOMICAL_HIGH_TIDES missing or invalid")
        YEAR_DATA = {}
